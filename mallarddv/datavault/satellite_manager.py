...
class DVSatelliteManager:
    ...
    def create_sat_from_metadata(
        ...
        records = self.hash_generator.groupby(raw_sat_records, keys=["rel_type", "base_name"])
        ...
        for rel_base_name, fields in records.items():
            ...
            try:
                if len(hk_list) != 1:
                    raise DVEntityError(f"Satellite {table_base_name} must have exactly one hub key")
                    
                _, error = self.db.execute_sql_safely(
                    sql_str,
                    description=f"Create {table_type}_{table_base_name}"
                )
                if error:
                    errors.append(error)
    ...
    def load_related_sats(
        ...
        for sat in sat_load:
            ...
            insert_new_sql = SQLTemplates.INSERT_SAT_NEW.format(
                ...
                hashdiff_field=f"{group_name}_hashdiff" if len(fields) else sat["source_field"],
            ...
            _, error = self.db.execute_sql_safely(
                insert_new_sql,
                description=f"Load satellite {sat_name} (new records)"
            )
            if error:
                errors.append(error)
