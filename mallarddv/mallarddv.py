...
class MallardDataVault:
    ...
    def __init__(self, database_path: str, scripts_path: Optional[str] = None) -> None:
        ...
        self.hub_manager = DVHubManager(self.db, self.metadata_manager, self.hash_generator)
        self.link_manager = DVLinkManager(self.db, self.metadata_manager, self.hash_generator)
        self.satellite_manager = DVSatelliteManager(self.db, self.metadata_manager, self.hash_generator)
    ...
    def init_mallard_db(
        ...
        if not meta_only:
            ...
            # Create satellites
            errors.extend(self.satellite_manager.create_sat_from_metadata(verbose=verbose))
    ...
    def create_sat_from_metadata(
        ...
        Returns:
            List of (SQL, error) tuples if errors occurred, otherwise empty list
        """
        return self.satellite_manager.create_sat_from_metadata(base_name, rel_type, verbose)
    ...
    def execute_flow(
        ...
        return self.flow_executor.execute_flow(
            source_table, record_source, file_path, load_date_overwrite, force_load, verbose
        )
