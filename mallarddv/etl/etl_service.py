...
class ETLService:
    ...
    def load_file_to_staging(
        self, source_table: str, file_path: str, file_type: Optional[str] = None, 
        verbose: bool = False
    ) -> List[Tuple[str, str]]:
        """
        ...
        _, error = self.db.execute_sql_safely(
            truncate_statement,
            description=f"Truncate staging table {source_table}"
        )
        if error:
            errors.append(error)
            return errors
        ...
        try:
            ...
            _, error = self.db.execute_sql_safely(
                load_statement,
                description=f"Load {file_path} into stg.{source_table}"
            )
            if error:
                errors.append(error)
