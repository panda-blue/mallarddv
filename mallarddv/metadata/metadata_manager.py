...
class MetadataManager:
    ...
    def get_transitions(self, source_table: str) -> List[Dict[str, Any]]:
        ...
        return self.db.fetch_dict(
            sql=SQLTemplates.GET_TRANSITIONS,
            params=[source_table]
        )
    ...
    def init_metadata_tables(self, verbose: bool = False) -> List[Tuple[str, str]]:
        ...
        _, error = self.db.execute_sql_safely(
            SQLTemplates.META_TRANSITIONS,
            description="Create metadata.transitions"
        )
        if error:
            errors.append(error)
        ...
        _, error = self.db.execute_sql_safely(
            SQLTemplates.META_RUNINFO,
            description="Create metadata.runinfo"
        )
        if error:
            errors.append(error)
    ...
    def overwrite_metadata_from_files(
        ...
        if meta_tables_path:
            ...
            if verbose:
                print(truncate_str)
            _, error = self.db.execute_sql_safely(truncate_str, description="Truncate metadata.tables")
            if error:
                errors.append(error)
            
            if verbose:
                print(insert_str)
            _, error = self.db.execute_sql_safely(insert_str, description="Insert into metadata.tables")
            if error:
                errors.append(error)
        ...
        if meta_transitions_path:
            ...
            if verbose:
                print(truncate_str)
            _, error = self.db.execute_sql_safely(truncate_str, description="Truncate metadata.transitions")
            if error:
                errors.append(error)
            
            if verbose:
                print(insert_str)
            _, error = self.db.execute_sql_safely(insert_str, description="Insert into metadata.transitions")
            if error:
                errors.append(error)
    ...
    def check_previous_ingestion(
        ...
        result = self.db.fetch_dict(
            SQLTemplates.CHECK_INGESTION,
            params=[file_path, source_table, status]
        )
        return len(result) > 0
