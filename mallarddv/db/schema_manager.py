...
class SchemaManager:
    ...
    def create_schemas(self, verbose: bool = False) -> List[Tuple[str, str]]:
        ...
        for schema in self.schema_list:
            ...
            _, error = self.db.execute_sql_safely(
                sql_str, description=f"Create schema {schema}"
            )
            if error:
                errors.append(error)
    ...
    def create_staging_table_from_metadata(
        ...
        for base_name, records in base_name_groups.items():
            ...
            _, error = self.db.execute_sql_safely(
                sql, description=f"Create staging table {base_name}"
            )
            if error:
                errors.append(error)
        ...
        for base_name, records in base_name_groups.items():
            ...
            for record in records:
                ...
                try:
                    ...
                    _, error = self.db.execute_sql_safely(
                        script, description=f"Apply script {script_path}"
                    )
                    if error:
                        errors.append(error)
